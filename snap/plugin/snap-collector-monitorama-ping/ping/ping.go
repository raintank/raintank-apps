...
func (p *Ping) CollectMetrics(mts []plugin.MetricType) ([]plugin.MetricType, error) {
	...
	if err != nil {
		LogError("ping collect err: ", "error", err)
		return nil, err
	}
...
func ping(checkId, agentName, endpoint, host string, mts []plugin.MetricType) ([]plugin.MetricType, error) {
	...
	err := check.Run()
	if err != nil {
		LogError("ping run err: ", "error", err)
		return nil, err
	}
	...
	for _, m := range mts {
		...
		if value, ok := stats[stat]; ok {
			mt := plugin.MetricType{
				Data_:      value,
				Namespace_: core.NewNamespace("monitorama", agentName, endpoint, "ping", stat),
				Timestamp_: time.Now(),
				Version_:   m.Version(),
			}
	...
	if state != lastState {
		...
		mt := plugin.MetricType{
			Data_:      message,
			Namespace_: core.NewNamespace("monitorama", "event", "monitor_state", stat),
...
func (p *Ping) GetMetricTypes(cfg plugin.ConfigType) ([]plugin.MetricType, error) {
	...
	for _, metricName := range metricNames {
		mts = append(mts, plugin.MetricType{
			Namespace_: core.NewNamespace("monitorama", "*", "*", "ping", metricName),
		})
	}
	return mts, nil
}
